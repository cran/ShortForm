...
lav_partable_full <- function (partable = NULL, 
                               ...
                               start = FALSE) {
  stopifnot(!is.null(partable$lhs), !is.null(partable$op), 
  ...
  if (length(ov.names.ord) > 0L) {
    tmp <- strsplit(vnames(partable, "th"), 
                    "\\|")
...
vnames <- function (partable, type = NULL, ..., warn = FALSE, ov.x.fatal = FALSE) 
{
  if (length(partable$lhs) == 0) 
    return(character(0L))
  dotdotdot <- list(...)
  type.list <- c("ov", "ov.x", "ov.nox", "ov.model", "ov.y", 
                 ...
                 "eqs.y", "eqs.x")
  stopifnot(is.list(partable), !missing(type), type %in% c(type.list, 
                                                           "all"))
  ...
  if (ndotdotdot > 0L) {
    ...
    for (dot in seq_len(ndotdotdot)) {
      ...
      if (is.null(partable[[block.var]])) {
        stop("lavaan ERROR: selection variable `", block.var, 
             " not found in the parameter table.")
      }
      else {
        if (!all(block.val %in% partable[[block.var]])) {
          stop("lavaan ERROR: ", block.var, " column does not contain value `", 
               block.val, "'")
        }
  ...
  for (b in block.select) {
    ...
    if (any(type %in% c("ov.x", "ov.nox", "ov.num", "ov.model", 
      ...
      vars <- c(partable$lhs[partable$block == b & partable$op == 
    ...
    if (any(type %in% c("ov.ord", "th", "th.mean", "ov.num", 
                        "lv.nonnormal"))) {
